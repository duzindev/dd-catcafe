...
Citizen.CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		exports.ox_target:addBoxZone({
			coords = v.location,
			size = vec3(v.width, v.height, v.maxZ - v.minZ),
			rotation = v.heading,
			debug = Config.Debug,
			options = {
				{
					event = "dd-catcafe:Chair",
					icon = "fas fa-chair",
					label = "Sit Down",
					loc = v.location,
					head = v.heading,
...
RegisterNetEvent('dd-catcafe:Chair', function(data)
	local canSit = true
	local sitting = false
	for k, v in pairs(QBCore.Functions.GetPlayersFromCoords(data.loc, 0.6)) do
		local dist = #(GetEntityCoords(GetPlayerPed(v)) - data.loc)
		if dist <= 0.4 then exports.qbx_core:Notify("Someone is already sitting there..", 'error') canSit = false end
	end
	if canSit then
